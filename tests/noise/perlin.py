...
from procgen.noise import perlin3D, combined
...
def performance2():
	...
	base = time()
	N = 70
	pmap = [[combined(perlin3D, x/N, y/N, 0, octaves = 50) for x in range(N)] for y in range(N)]
	elapsed = time() - base
	print("\tElapsed %s seconds" % elapsed)
	assert elapsed < 3
...
def subjective():
	...
	plt.subplot(222)
	plt.title("1 octave")
	pmap = [[combined(perlin3D, 6*x/N, 6*y/N, 0, octaves = 1) for x in range(N)] for y in range(N)]
	plt.imshow(pmap, cmap = 'plasma', interpolation = 'nearest')
	
	plt.subplot(223)
	plt.title("4 octaves, marbled")
	mpmap = [[sin(1.6 * 2 * pi * combined(perlin3D, 6*x/N, 6*y/N, 0, octaves = 4)) for x in range(N)] for y in range(N)]
	plt.imshow(mpmap, cmap = 'plasma', interpolation = 'nearest')
	
	plt.subplot(224)
	plt.title("15 octaves, crinkled")
	cpmap = [[combined(perlin3D, 10*x/N, 10*y/N, .2, octaves = 15) for x in range(N)] for y in range(N)]
